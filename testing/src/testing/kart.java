...
public class kart {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for (int i = 0; i < products.size(); i++) {
			...
			if(itemList.contains(name)) {  //text.contains("Cucumber") ---> for single object this will be the condition
			 j++;
			...
			//driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
			  if(j == items.length) //to stop searching if all the item in arraylist have been added to cart, length is used in array and size() is used in arraylist
				  break;
		...
		w.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[text()='Code applied ..!']")));  //----------> explicit wait
